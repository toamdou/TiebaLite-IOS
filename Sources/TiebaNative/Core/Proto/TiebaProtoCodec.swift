...
final class TiebaProtoRegistry: @unchecked Sendable {
  ...
  /// 描述符随 app bundle 发布（单一真源，不再由 JS 注入）。读不到就是构建坏了，
  /// 大声失败——不能让映射层在空注册表上静默产出错误数据。
  func loadBundled() {
    guard
      let url = Bundle.main.url(forResource: "proto-descriptors", withExtension: "json"),
      let json = try? String(contentsOf: url, encoding: .utf8),
      (try? initialize(json: json)) != nil
    else {
      preconditionFailure("proto-descriptors.json 缺失或无法解析")
    }
  }
